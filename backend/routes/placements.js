...
router.get("/", async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT p.*, sa.student_id, s.email AS student_email, s.dept,
...
router.post("/", async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      "INSERT INTO Placement (application_id, job_id, ctc, join_date) VALUES (?, ?, ?, ?)",
      [application_id, job_id, ctc, join_date]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    await db.query(
      "UPDATE Placement SET application_id=?, job_id=?, ctc=?, join_date=? WHERE placement_id=?",
      [application_id, job_id, ctc, join_date, req.params.id]
    );
